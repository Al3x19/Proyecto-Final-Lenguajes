...
namespace TiendaSoftware.Controllers
{
    ...
    public class ReviewController : ControllerBase
    {
        ...
        [HttpGet("{id}")]
        public async Task<ActionResult<ResponseDto<ReviewDto>>> GetOneById(string id, Guid SoftwareId)
        {
            var response = await _reviewsService.GetByIdAsync(id, SoftwareId);
        ...
        [HttpGet("software/{id}")]
        public async Task<ActionResult<ResponseDto<ReviewDto>>> GetBySoftwareId(Guid id, int page = 1)
        {
            var response = await _reviewsService.GetBySoftwareIdAsync(id, page);
