...
namespace TiendaSoftware.Services
{
    public class PublisherService : IPublisherService
    {
        ...
        public PublisherService(IAuditService auditService, TiendaSoftwareContext context, IMapper mapper, IConfiguration configuration)
        {
        ...
        public async Task<ResponseDto<PaginationDto<List<PublisherDto>>>> GetPublishersListAsync(string searchTerm = "", int page = 1)
        {
            ...
            if (!string.IsNullOrEmpty(searchTerm))
            {
                ...
                publisherEntityQuery = publisherEntityQuery.Where(x =>
                    (x.Name + " " +  x.CreatedByUser.UserName + " " + x.Description).ToLower().Contains(normalizedSearchTerm)
                );
            }
        ...
        public async Task<ResponseDto<PublisherDto>> CreateAsync(PublisherCreateDto dto)
        {
            ...
            var AlreadyOwner = await _context.Publishers.AnyAsync(x => x.CreatedBy == UserId);
            ...
            if (AlreadyOwner)
            {
                return new ResponseDto<PublisherDto>
                {
                    StatusCode = 405,
                    Status = false,
                    Message = "Ya administra una cuenta de desarrollador..."
                };
            }
        ...
        public async Task<ResponseDto<PublisherDto>> EditAsync(PublisherEditDto dto, Guid id)
        {
            ...
            if (publisherEntity.CreatedBy != UserId)
            {
                return new ResponseDto<PublisherDto>
                {
                    StatusCode = 401,
